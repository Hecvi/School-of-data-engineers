...
'''
Выполняем скрипт "delete_tables.sql" на создание таблиц из СУБД
'''


def create_tables(con, sql_ls):
    curs = con.cursor()
    for elem in sql_ls:
        curs.execute(elem)
    con.commit()
...
if __name__ == "__main__":
    ...
    sql_ls = sql_script_parser('/home/itde1/HECV/sql_scripts/create_tables.sql')
    create_tables(con, sql_ls)
    con.close()